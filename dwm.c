...
static unsigned int blocknum; // blocks idx in mouse clicks
static unsigned int stsw = 0; // status width
static unsigned int numlockmask = 0;
static unsigned int sleepinterval = 0, maxinterval = 0, count = 0;
static unsigned int execlock = 0; // ensure only one child process exists per block per instance
...
#endif
         {
            if (*blockoutput[i] == '\0') { continue; } // ignore cmd that outputs NULL or \0
